...
def main():
    ...
    if len(sys.argv) < 4:
        printf(
            "chfont_demo <cp936-ttc-font-file-name> "
    ...
    fcp936_name = HPDF_LoadTTFontFromFile2(
        pdf, sys.argv[1], int(sys.argv[2]), HPDF_TRUE
    )
    fcp932_name = HPDF_LoadTTFontFromFile2(
        pdf, sys.argv[3], int(sys.argv[4]), HPDF_TRUE
    )
