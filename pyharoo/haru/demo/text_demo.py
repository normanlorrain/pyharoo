...
from pyharoo.haru.c_func import *
from pyharoo.haru.hpdf_errorcode import *
...
def main():
    ...
    HPDF_Page_SetLineWidth(page, 1)
    HPDF_Page_Rectangle(
        page, 50, 50, HPDF_Page_GetWidth(page) - 100, HPDF_Page_GetHeight(page) - 110
    )
    HPDF_Page_Stroke(page)
    ...
    while fsize < 60:
        ...
        length = HPDF_Page_MeasureText(
            page, samp_text, HPDF_Page_GetWidth(page) - 120, HPDF_FALSE, NULL
        )
    ...
    HPDF_Page_SetTextMatrix(
        page, cos(rad1), sin(rad1), -sin(rad1), cos(rad1), 330, ypos - 60
    )
